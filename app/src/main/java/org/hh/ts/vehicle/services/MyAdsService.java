...
public class MyAdsService extends IntentService {
	...
	private boolean updateAdId() {
		...
		BufferedInputStream bis = null;
		try{
			File adsFile = new File(this.getFilesDir().getPath()+File.separator+ADS_XML_FILENAME);
			...
			while(eventType!=XmlPullParser.END_DOCUMENT && adUnitId == null){
				...
				case XmlPullParser.START_TAG:
					if(AdsXml.NODE_CATEGORY.equals(parser.getName())) {
						if(parser.getAttributeValue("", AdsXml.ATTR_NAME).equals(getString(R.string.ads_xml_category_name))) {
						...
						if(AdsXml.NODE_AD.equals(parser.getName())) {
							if(getString(R.string.ads_xml_ad_name).equals(parser.getAttributeValue("", AdsXml.ATTR_NAME))) {
